...
void		do_specularity(t_shorter *sh, int spec, t_rtv *rtv, t_light *light)
{
	if (spec != -1)
	{
		sh->vec_r = 2.0 * dot(rtv->calc.norml, sh->l_ray) *
		rtv->calc.norml - sh->l_ray;
		sh->r_dot_v = dot(sh->vec_r, rtv->calc.dir);
		if (sh->r_dot_v > 0)
			sh->i += light->intensity * pow(sh->r_dot_v / (length(sh->vec_r)
			* length(rtv->calc.dir)), spec);
	}
}
...
double		do_calc_lightning(t_rtv *rtv, t_vec_d p, int spec, t_light **l)
{
	...
	if (light->type == POINT)
		...
		sh.l_ray = light->dir;
	sh.calc = rtv->calc;
	clos_intersection(to_calc(p, sh.l_ray, EPSILON, INFINIT),
	&sh.calc, rtv);
	...
	if (sh.n_dot_l > 0)
		sh.i += light->intensity * sh.n_dot_l / (length(rtv->calc.norml)
		* length(sh.l_ray));
...
double		calc_lightning(t_rtv *rtv, t_vec_d p, int spec)
{
	...
	light = rtv->lights;
	len_normal = length(rtv->calc.norml);
	i = 0.0;
	while (light)
	{
